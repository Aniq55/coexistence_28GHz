...
R_v = 1/(2*sqrt(lambda_c));

R_vec = [50: 50: 500]; % exclusion zone radius
length(R_vec)
P_ex_vec = zeros(1, length(R_vec));
P_in_vec = zeros(1, length(R_vec));
D_vec = zeros(1, length(R_vec));
...
%%
I_2 =  vpaintegral( (1 - S_1).*x, x, [R Inf]); % [OK]
...
if R < R_v
    ...
    L_Ic = exp(-2.*pi.*lambda_c.*exp(-pi.*lambda_s.*(R - R_v).^2).*I_3); % [OK]
end

L_Ic_in = exp(-2.*pi.*lambda_c.*I_3);
...
tau_val = tau_vec_abs(2);
S_vec = zeros(1, iter);
S_in_vec = zeros(1, iter);
...
for i= 1:iter
    ...
    try
        ...
        S_in_vec(i) = vpa(subs(L_Ic_in, {tau, v}, {tau_val, v_val_vec(index)}));
...
P_ex = mean(S_vec(~isnan(S_vec)));
P_in = mean(S_in_vec(~isnan(S_in_vec)));
...
p_point_ex = exp(-lambda_s*pi*R^2);
p_point_in = 1 - p_point_ex;

P_ex_vec(k) = P_ex;
P_in_vec(k) = P_in;

D_vec(k) = (B_ex*p_point_ex*P_ex + B_in*p_point_in*P_in)*log2(1 + tau_val);
...
figure;
hold on;
plot(R_vec, P_ex_vec, 's-', 'LineWidth', 1.5);
plot(R_vec, P_in_vec, 's-', 'LineWidth', 1.5);
xlabel('Exclusion zone radius, R [m]')
...
%%
figure;
plot(R_vec, D_vec*1e-6, 's-', 'LineWidth', 1.5)
xlabel('Exclusion zone radius, R [m]')
ylabel('Average data rate D [Mbps]')
grid on; box on;
ylim([0, 150])

% TASK: save and plot P_ex, P_in
