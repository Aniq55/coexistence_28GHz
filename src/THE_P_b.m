...
%         
%         L_Ib_1 = (Theta/pi).*exp(-2.*pi.*lambda_b_tilde.*I_b_1_g1) ...
%             + (1 - Theta/pi).*exp(-2.*pi.*lambda_b_tilde.*I_b_1_g0); % [OK]
%         
%         L_Ib_2 = (Theta/pi).*exp(-2.*pi.*lambda_b_tilde.*I_b_2_g1) ...
%             + (1 - Theta/pi).*exp(-2.*pi.*lambda_b_tilde.*I_b_2_g0); % [OK]
%         
%         L_Ib_3 = (Theta/pi).*exp(-2.*pi.*lambda_b_tilde.*I_b_3_g1) ...
%             + (1 - Theta/pi).*exp(-2.*pi.*lambda_b_tilde.*I_b_3_g0); % [OK]
        ...
        I_3_1 = vpaintegral(S_2_1, x, [1 Inf]); % [OK]
        I_3_2 = vpaintegral(S_2_2, x, [1 Inf]); % [OK]
        I_3_3 = vpaintegral(S_2_3, x, [1 Inf]); % [OK]
        
        L_Ic_1 = exp(-2.*pi.*lambda_c_tilde.*I_3_1); % [OK]
        L_Ic_2 = exp(-2.*pi.*lambda_c_tilde.*I_3_2); % [OK]
        L_Ic_3 = exp(-2.*pi.*lambda_c_tilde.*I_3_3); % [OK]
        ...
        F_v = @(z) exp(-pi.*lambda_b_tilde.*z.^2);
        ...
        %% Save P_b_vec
        csvwrite('D:\Satellites\28GHz\data\coverage\P_b_'...
            +string(int32(h*1e-3))+'_'+string(N) ...
            +'_'+string(R_s) + '_'+string(R_b) +'.csv', P_b_vec);
