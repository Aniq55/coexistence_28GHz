    ...
    for R_b = Radius_list
        ...
        for iter = 1:iter_count
            ...
            if min_ES_dist>R_s
                ...
                I_b = sum(I_b_vec) - I_b_vec(index) + S_b;
                ...
                SIR_vec_b(valid_iter) = S_b/(I_b -S_b + I_c + I_s);
        ...
        SIR_vec_b_valid = SIR_vec_b(1:valid_iter);

        for i=1:length(tau_vec_dB)
            tau = tau_vec_abs(i);
            P_b_vec(i) = sum(SIR_vec_b_valid > tau)./length(SIR_vec_b_valid);
        end
        ...
        N_records = length(tau_vec_dB);
        tau_vec_abs = 10.^(tau_vec_dB./10.0);
        P_bn_vec = zeros(1, length(tau_vec_abs));

        for i=1:length(tau_vec_dB)
            tau = tau_vec_abs(i);
            P_bn_vec(i) = sum(SIR_vec_b > tau)./length(SIR_vec_b);
        end
        ...
        N_records = length(tau_vec_dB);
        tau_vec_abs = 10.^(tau_vec_dB./10.0);
        P_cn_vec = zeros(1, length(tau_vec_abs));

        for i=1:length(tau_vec_dB)
            tau = tau_vec_abs(i);
            P_cn_vec(i) = sum(SIR_vec_c > tau)./length(SIR_vec_c);
        end
        ...
        DATA = [R_s_vec, R_b_vec, P_c_vec', P_b_vec', P_cn_vec', P_bn_vec'];
